...
Blockly.Blocks['output_auto'] = {
  ...
  mutationToDom: function() {
    this.itemCount_ = this.countPercent(this.getFieldValue('TEXT'));
  ...
  countPercent: function(str) {
    return str.replace(/%%/g, '').split('%').length -  1;
  },

  // ==============  追加 (2017/11/15) ===========================
  validator: function(text) {    
    var target = /* this.getText*/ (text);      // 入力文字を動的に記録する
    this.sourceBlock_.itemCount_ = this.sourceBlock_.countPercent(target); 
    //this.updateShape_();
