...
Blockly.Blocks['controls_switch'] = {
  ...
  decompose: function(workspace) {
    ...
    if (this.defaultCount_) {
      var defaultBlock = new Blockly.Block(workspace, 'controls_switch_default');
    ...
    while (clauseBlock) {
      switch (clauseBlock.type) {
        ...
        case 'controls_switch_default':
          ...
          defaultInput.appendTitle('default');
          // Reconnect any child blocks.
          if (clauseBlock.statementConnection_) {
  ...
  saveConnections: function(containerBlock) {
    ...
    while (clauseBlock) {
      switch (clauseBlock.type) {
        case 'controls_switch_case':
          var inputCase = this.getInput('CASE' + x);
          var inputDo = this.getInput('DO' + x);
          clauseBlock.valueConnection_ =
...
Blockly.Blocks['quiz3_if0'] = {
  init: function() {
    this.setColour(210);
    this.appendDummyInput()
        .appendTitle("if ( arr[i] > 60 ) {");
    this.appendStatementInput("DO1");
    this.appendDummyInput()
        .appendTitle("} else if ( arr[i] == 60 ) {");
