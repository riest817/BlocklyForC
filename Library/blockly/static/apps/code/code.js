...
Code.PAGE = BlocklyApps.getNumberParamFromUrl('page', 1, Code.MAX_PAGE);
Code.MAX_BLOCKS = [undefined, // Level 0.
    Infinity, Infinity, 2, 5, 5, 5, 5, 10, 7, 10][Code.PAGE];
 ...
 * Populate the currently selected pane with content generated from the blocks.
 現在選択されているパネルにブロックから生成されたコンテンツを取り込みます。
 */
 ...
 */
      ...
      '<xml>' +
        '<block type="quiz_start_main" x="0" y="0">' +
          '<statement name="DO">' +
            '<block type="quiz_declaration1_def">' +
              '<next>' +
                '<block type="quiz_scanf_n">' +
                  '<next>' +
                    '<block type="quiz3_for">' +
                      '<statement name="DO">' +
                        '<block type="quiz_scanf_arr"></block>' +
                      '</statement>' +
                      '<next>' +
                        '<block type="quiz3_for">' +
                          '<statement name="DO">' +
                            '<block type="quiz_frame">' +
                              '<statement name="DO">' +
                                '<block type="quiz3_if0">' +
                                  '<statement name="DO1">' +
                                    '<block type="quiz3_printf1"></block>' +
                                  '</statement>' +
                                  '<statement name="DO2">' +
                                    '<block type="quiz3_printf1"></block>' +
                                  '</statement>' +
                                  '<statement name="DO3">' +
                                    '<block type="quiz3_printf2"></block>' +
                                  '</statement>' +
                                '</block>' +
                              '</statement>' +
                            '</block>' +
  ...
  function numberSwitch() {
    document.getElementById('quizNumber').innerHTML = '問題1-' + Code.PAGE;
  }
  Blockly.addChangeListener(onchange);
  Blockly.addChangeListener(numberSwitch);
...
//alert(Blockly.mainWorkspace.remainingCapacity());
 ...
 /*var judg = '#include  <stdio.h>\n\n' + 
  ...
  var ans = Code.decision(Code.PAGE);
  //window.alert(ans);
  if ( c0de == ans ){
  Code.congratulations();  
  } else {
...
Code.congratulations = function() {
  ...
  var style = {
    ...
    var text = BlocklyApps.getMsg('Code_nextLevel')
        .replace('%1', Code.PAGE + 1);                   //window open : Congratulations! Are you ready to proceed to page %1?  template.soyにて変更
 ...
 * Enter and space move to the next page, escape does not.
...
Code.nextLevel = function() {
  window.location = window.location.protocol + '//' +
      ...
      '?lang=' + BlocklyApps.LANG + '&page=' + (Code.PAGE + 1);
};
...
Code.feild = function() {
  ...
  var style = {
    ...
    var text = "間違いです。";                   // 不正解の場合のテキスト(window)
    var cancel = document.createElement('button');
    cancel.appendChild(
        document.createTextNode(BlocklyApps.getMsg('dialogCancel')));
    cancel.addEventListener('click', BlocklyApps.hideDialog, true);
