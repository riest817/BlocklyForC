...
Blockly.C.ORDER_FUNCTION_CALL = 1;    //
Blockly.C.ORDER_DOT = 2;              // . (right), !! (left)
Blockly.C.ORDER_POWER = 3;            // ^, ^^, **
Blockly.C.ORDER_MULTIPLICATION = 4;   // *, /, `quot`, `rem`, `div`, `mod`, %, :%
Blockly.C.ORDER_ADDITION = 5;         // +, -
Blockly.C.ORDER_CONS = 6;             // :, ++
Blockly.C.ORDER_EQUALITY = 7;         // == != > >= < <=
Blockly.C.ORDER_LOGICAL_AND = 8;      // &&
Blockly.C.ORDER_LOGICAL_OR = 9;       // ||
Blockly.C.ORDER_BIND = 10;            // >>, >>=
Blockly.C.ORDER_DNIB = 11;            // =<<
Blockly.C.ORDER_SEQ  = 12;            // $, $!, `seq`
// ``ambiguities that are to be resolved by making grammatical phrases as long as possible''
Blockly.C.ORDER_IF   = 50;            // if then else , case of, let in, \ ->,  
...
Blockly.C.ORDER_OVERRIDES = [
  ...
  // foo. (bar . baz) -> foo . bar . baz
  [Blockly.C.ORDER_DOT, Blockly.C.ORDER_DOT],
  // a ^ (b ^ c) -> a ^ b ^ c
  [Blockly.C.ORDER_POWER, Blockly.C.ORDER_POWER],
  // (a * b) * c -> a * b * c
  [Blockly.C.ORDER_MULTIPLICATION, Blockly.C.ORDER_MULTIPLICATION],
  // (a + b) + c -> a + b + c
  [Blockly.C.ORDER_ADDITION, Blockly.C.ORDER_ADDITION],
  // a : (b : c) -> a : b : c
  [Blockly.C.ORDER_CONS, Blockly.C.ORDER_CONS],
  // a && (b && c) -> a && b && c
  [Blockly.C.ORDER_LOGICAL_AND, Blockly.C.ORDER_LOGICAL_AND],
  // a || (b || c) -> a || b || c
  [Blockly.C.ORDER_LOGICAL_OR, Blockly.C.ORDER_LOGICAL_OR],
  // (a >> b) >> c -> a >> b >> c
  [Blockly.C.ORDER_BIND, Blockly.C.ORDER_BIND],
  // a =<< (b =<< c) -> a =<< b =<< c
  [Blockly.C.ORDER_DNIB, Blockly.C.ORDER_DNIB],
  // a $ (b $ c) -> a $ b $ c
  [Blockly.C.ORDER_SEQ, Blockly.C.ORDER_SEQ],
];
    ...
    for (var i = 0; i < block.inputList.length; i++) {
      if (block.inputList[i].type == Blockly.INPUT_VALUE) {
        ...
        if (childBlock) {
          ...
          if (comment) {
            commentCode += Blockly.C.prefixLines(comment, '-- '); // 17/12/05 ↑と同様
          }
        }
      }
    }
 ...
 */
Blockly.C.statementToCodeWithSeparator = function(block, name, sep) {
 ...
 */
Blockly.C.blockToCodeWithSeparator = function(block, separator) {
  if (!block) {
    return '';
  }
  if (block.disabled) {
 ...
 * @private
 */
Blockly.C.scrubWithSeparator = function(block, code, separator) {
  var commentCode = '';
  // Only collect comments for blocks that aren't inline.
  if (!block.outputConnection || !block.outputConnection.targetConnection) {
    // Collect comment for this block.
